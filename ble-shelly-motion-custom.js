...
let CONFIG = {
  ...
  allowedMacAddresses: [
    "3c:2e:f5:bc:ee:dd", // events only from these mac addresses are allowed.
  ],
  globalMqttTopic: "shellies/shellymotionsensor-",
  motionHandler: function (motion, eventData) {
    let device = eventData.address.split(':').join('');
    let topic = CONFIG.globalMqttTopic + device + "/motion";
    MQTT.publish(topic, String(motion));
    console.log("Motion",eventData);
    CONFIG.batteryHandler(eventData.battery, eventData);
  },
  batteryHandler: function (battery, eventData) {
    let device = eventData.address.split(':').join('');
    let topic = CONFIG.globalMqttTopic + device + "/battery";
    MQTT.publish(topic, String(battery));
    console.log("Battery",eventData);
  },
  illuminanceHandler: function (illuminance, eventData) {
    let device = eventData.address.split(':').join('');
    let topic = CONFIG.globalMqttTopic + device + "/illuminance";
    MQTT.publish(topic, String(illuminance));
    console.log("Illuminance",eventData);
    CONFIG.batteryHandler(eventData.battery, eventData);
  },
  /**
